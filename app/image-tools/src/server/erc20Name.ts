...
function getRpcUrlFromEnv(chainId: number): string | undefined {
	const keys = [`RPC_URI_FOR_${chainId}`, `RPC_${chainId}`, `VITE_RPC_URI_FOR_${chainId}`, `VITE_RPC_${chainId}`];
	const val = keys.map(key => process.env[key]).find(Boolean);
...
export async function handleErc20Name(req: Request): Promise<Response> {
	...
	try {
		...
		if (!rpc)
			return new Response(JSON.stringify({error: 'No RPC configured for chain'}), {
				status: 400,
				headers: {'Content-Type': 'application/json'}
			});
		...
		if (!r.ok) {
			...
			return new Response(JSON.stringify({error: `RPC HTTP ${r.status}`, details: bodyText?.slice?.(0, 300)}), {
				status: 502,
				headers: {'Content-Type': 'application/json'}
			});
		}
